...
-- I honestly have no idea. This feels like an if statement kind of deal. Have we gone over that?
...
WHERE p.payment_date > "2005-08" AND p.payment_date <"2005-09"
...
SELECT c.first_name, c.last_name, SUM(p.amount) AS paid
FROM customer c
JOIN payment p
...
SELECT f.title, COUNT(f.film_id) AS count_of_rentals
FROM film f
JOIN inventory i
